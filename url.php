<?php
  /*
Основной скрипт, работающий по GET-запросам:
параметр action: get - перейти по адресу, заданному меткой в параметре url,
add - добавить адрес, заданный в параметре url в базу, вернуть ссылку. Если
задано поле user_url, то присвоить заданную им метку.

По умолчанию за метку считается поле id соответствующей записи в таблице, перевёденное в основание 62
и закодированное символами латиницы (заглавными и строчными) и цифрами.

В таблице urls_table хранятся адреса, каждому адресу присвоен уникальный id.

В таблице users_urls хранятся пользовательские метки (поле user_url) с id соответствующей строчки из urls_table (поле pointer)

*/

include "functions.php"; // файл с прикладными функциями
...
if ($action == "get"){
  // описание редиректа по адресу, который связан с меткой
  ...
  // описание добавление новой пары метка-адрес
  ...
  if (preg_match("/^$regex_url$/", $url)) {
    ...
    // обработка пользовательской метки (её может и не быть)
    if ($user_url != "") {
